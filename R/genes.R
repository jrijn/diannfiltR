...
#' @param filter_q_value numeric. GG.Q.Value threshold for Gene group filtering (default: 0.01). Set to NULL to disable filtering.
#'
#' @return data.frame. Filtered gene data
#' @export
#' @importFrom dplyr filter group_by summarise n_distinct left_join across all_of mutate
#' @importFrom tidyr replace_na
genes <- function(diann_data, proteotypic = TRUE, min_peptides = 2, remove_contaminants = FALSE, filter_q_value = 0.01) {
  # Validate input
